...
class ConfigHeader(QObject):
    ...
    def __init__(self, header):
        ...
        # 注册也用用哈
        self.header.loginBox.connectLogin2(self.regiseter)
        self.loginInfor = {}
        self.result = None
        ...
        # self.loadCookies()
    def regiseter(self):
        self.header.loginBox.reg_LoginInformation()
        # pass
    def bindConnect(self):
    ...
    def loadLoginInformations(self, informations:tuple):
        ...
        ConfigWindow.result = informations
        self.loginInfor = ConfigWindow.result
        print('232 self.loginInfor:   ', self.loginInfor)
        # 网络不通或其他问题。
        # if not result:
        ...
        #     self.code = 500
        #     return
        data = pd.read_csv('user.csv', names=['用户名', '密码'], header=0, encoding='utf-8', sep=",")

        if self.loginInfor[0] in list(data['用户名']):
            flag1 = 3


            # print("密码",data[data["用户名"] == self.user_id]["密码"])
            # print(str(data[data["用户名"] == self.loginInfor[0]]["密码"]))
            if self.loginInfor[-1] == str(data[data["用户名"] == self.loginInfor[0]]["密码"].iloc[0]):
                print("登录成功")
                # self.window = ENIACMusic()
                # self.close()
                # self.window.show()
                df = pd.read_csv('user.csv', names=['用户名', '密码'], header=0, encoding='utf-8', sep=",")
                for i in range(df.shape[0]):
                    line = list(df.iloc[i])
                    if line[0] == self.loginInfor[0]:
                        global is_login
                        is_login = True
                        self.code = 200
                        return
            else:
                QMessageBox.information(self.header,"注意", "密码错误")

        else:
            QMessageBox.information(self.header, "注意", "账号不存在")
    ...
    def loginFinished(self):
        ...
        if str(self.code) == '200':
            ...
            # self.setUserData()
            self.header.loginButton.setText(self.loginInfor[0])
