...
import configRecommendFrameFeatures
import downloadFrame
import re
import os
import sys
import random
...
from PyQt5.QtGui import QBrush, QColor, QCursor, QPixmap, QFont, QPainter, QPen, QLinearGradient, QIcon
...
class PlayWidgets(QFrame):
    ...
    def setButtons(self):
        ...
        # icon = QtGui.QIcon()
        self.lyricButton.setIcon(QIcon(QPixmap("resource/收藏.png")))
        # self.lyricButton.setIconSize(QtCore.QSize(15, 15))
        self.lyricButton.setToolTip("收藏")
        self.lyricButton.setStyleSheet('''
                      border:none;
        ''')
    ...
    def lyricEvent(self):
        if self.lyricMode == 0:
            self.lyricButton.setIcon(QIcon(QPixmap("resource/收藏1.png")))
            data = configDownloadFrameFeatures.ConfigDownloadFrame.list
            song_name = data['name']
            ...
            try:
                ...
                print(e)
            df = pd.read_csv('like.csv', names=['user','name', 'author','time','url'], header=0, encoding='utf-8', sep=",")
            df2 = pd.DataFrame([[user_name,song_name,author_name,time,song_url]], columns=['user','name', 'author','time','url'])
            df3 = df.append(df2)
            ...
            #     if self.username == '':
            #         return
            df3.to_csv('like.csv', encoding='utf-8')
            print(song_name,author_name,time)

            self.lyricMode = 1

        elif self.lyricMode == 1:
            data = configDownloadFrameFeatures.ConfigDownloadFrame.list
            song_name = data['name']
            author_name = data['author']
            time = data['time']
            try:
                ...
                print(e)
            df4 = pd.read_csv('like.csv', names=['user','name', 'author','time','url'], header=0, encoding='utf-8',
                             sep=",")
            ...
            for index,i in df4.iterrows():
                ...
                # print(item[0])
                # print(item[1])
                df5 = pd.DataFrame(i, columns=['user','name', 'author','time','url'])
                df6 = df4.append(df5)
                df6.reset_index(drop=True)
                df6.to_csv('like.csv', encoding='utf-8')
                if str(item[0]) == user_name and str(item[1]) == song_name:
                    try:
                        list = [user_name,song_name]
                        data1 = df6[df6['user'] == user_name]
                        data2 = df6[df6['name'] == song_name]
                        data = df6[(df6['user'] == user_name) & (df6['name'] == song_name)]
                        i = data.index.tolist()
                        df7 = df6.drop(i)
                        df7.to_csv('like.csv', encoding='utf-8')
                    except Exception as  e:
                        print(e)
            self.lyricButton.setIcon(QIcon(QPixmap("resource/收藏.png")))
            self.lyricMode = 0

    def judcollection(self):
        """用于判断是否已经推荐"""
        data = configDownloadFrameFeatures.ConfigDownloadFrame.list
        song_name = data['name']
        author_name = data['author']
        user_name = ConfigWindow.result[0]
        coldf = pd.read_csv('like.csv', names=['user','name', 'author','time','url'], header=0, encoding='utf-8',
                             sep=",")
        rowCount = coldf.shape[0]
        for index,i in coldf.iterrows():
            item = i.tolist()
            # judcol = pd.DataFrame(i,columns=['user','name', 'author','time','url'])
            if str(item[0]) == user_name and str(item[1]) == song_name:
                    # judcoldf = judcol[(judcol['用户名'] == user_name)]
                    # jud = judcoldf[(judcoldf['音乐标题']==song_name)]
                    # if song_name==jud:
                        self.lyricButton.setIcon(QIcon(QPixmap("resource/收藏1.png")))
                        self.lyricMode = 1
            else :
...
class CurrentMusicShort(QFrame):
    ...
    def setLabels(self):
        ...
        self.musicPic = PicLabel('resource/q.png', 64, 64)
    ...
    def init(self):
        ...
        self.musicName.setText("正在播放")
        self.musicAuthor.setText("  ")
...
class Player(QMediaPlayer):
    ...
    def playMusic(self, url=None, data=None):
        ...
        if url:
            ...
            self.playWidgets.playEvent(self)
            self.playWidgets.judcollection()
